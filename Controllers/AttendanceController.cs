...
namespace WebApplication5.Controllers
{
    ...
    public class AttendanceController : Controller
    {
        ...
        // REPORT
        [HttpGet("", Name = "Attendance_Index")]
        public async Task<IActionResult> Index(int activityId, DateTime? startDate, DateTime? endDate, int? page)
        {
            var activity = await _context.Activity.FindAsync(activityId);
            if (activity == null) return NotFound();

            var sessionsQuery = _context.AttendanceSessions.Where(s => s.ActivityId == activityId);
            if (startDate.HasValue) sessionsQuery = sessionsQuery.Where(s => s.Date.Date >= startDate.Value.Date);
            if (endDate.HasValue) sessionsQuery = sessionsQuery.Where(s => s.Date.Date <= endDate.Value.Date);

            var sessions = await sessionsQuery.OrderBy(s => s.Date).ToListAsync();
            var dates = sessions.Select(s => s.Date.Date).ToList();
            var sessionIds = sessions.Select(s => s.Id).ToList();

            var records = await _context.AttendanceRecords
                .Where(r => sessionIds.Contains(r.AttendanceSessionId))
                .Select(r => new { r.AttendanceSessionId, r.UserId })
                .ToListAsync();
            var presentPairs = new HashSet<(int sessionId, string userId)>(records.Select(r => (r.AttendanceSessionId, r.UserId)));
            ...
            var memberRows = members.Select(m => new MemberAttendanceRow
            {
                Name = m.UserName,
                AttendancePerDate = dates.Select(d =>
                {
                    var sessionId = sessions.First(s => s.Date.Date == d).Id;
                    return presentPairs.Contains((sessionId, m.Id));
                }).ToList()
            }).ToList();

            // paginate members
            const int pageSize = 10;             // adjust to taste
            var pageNumber = page ?? 1;
            var pagedMembers = memberRows.ToPagedList(pageNumber, pageSize);
